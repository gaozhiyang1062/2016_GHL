    ...
    while True:
        if (index <= 500):
            ...
            for i, d in enumerate(dets):
                ...
                # 数据集增强，随机调整图片的对比度与亮度
                face = re_img(face, random.uniform(0.9, 1.8), random.randint(-10, 80))
...
def people_login():
    if 'person_'+name+'.jpg' in person_all:
        ...
        #p = str(len(person_all) + 1)
        path = 'person_' + name
        if not os.path.exists(path):
            os.makedirs(path)
        print("Start take face_img")
        get_pface_img(name, path)
        print('Get all face_img OK,thank you!')
    #other_face = get_other()
